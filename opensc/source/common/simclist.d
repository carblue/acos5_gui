...
Content covered by this file is ALL of header C/common/simclist.h

ALL extern(C) functions are exported from libsmm-local.so/.dll (PATCH_EXPORTS),
except those controlled by SIMCLIST_DUMPRESTORE.
Regular builds of libsmm-local.so/.dll don't #define SIMCLIST_DUMPRESTORE

simclist_restrict (C's restrict keyword) got translated to scope in D in the following, which is not an exact equivalent
...
/* WARNING
 * There is an issue with struct list_t, which is used in a member variable of struct sc_context.
 * The list_t.sizeof depends on whether version=SIMCLIST_WITH_THREADS is defined or not; so does sc_context.sizeof etc. too !
 * Of course, version=SIMCLIST_WITH_THREADS or no version setting has to match the 'gcc ... -D?' when building the libopensc and libsmm-local binary,
 * otherwise crash or undefined behavior may happen when using sc_context beyond .preferred_language.
 */
...
/*
SIMCLIST_DUMPRESTORE is also a #def or #undef macro in OpenSC;
Whether it was defined or (probably) not when building the OpenSC binary, shall have the same meaning for version identifier SIMCLIST_DUMPRESTORE
*/
version(SIMCLIST_DUMPRESTORE)
	struct list_dump_info_t {
		ushort version_;       /* dump version */
		long timestamp;      /* when the list has been dumped, microseconds from UNIX epoch */
		uint list_size;
		uint list_numels;
...
extern(C) nothrow
{
	 ...
	 */
	alias element_serializer = void* function(scope const(void)* el, scope uint* serialize_buffer);
	 ...
	 */
	alias element_unserializer = void* function(scope const(void)* data, scope uint* data_len);
} // extern(C) nothrow

/* [private-use] list entry -- holds actual user data */
struct list_entry_s {
	void*         data;
...
//			"spareels",
		...
		try {
			...
			foreach (i, ref member; this.tupleof) {
				mixin(head_foreach_Pointer);
				if (isDereferencable && canFind(pointersOfInterest, name_member)) {
					if (name_member=="head_sentinel")
						sink.formatValue(*head_sentinel, fmt);
					else if (name_member=="tail_sentinel")
						sink.formatValue(*tail_sentinel, fmt);
					else if (name_member=="mid")
						sink.formatValue(*mid, fmt);
					else if (name_member=="spareels")
						sink.formatValue(*spareels, fmt);
					else if (name_member=="iter_curentry")
						sink.formatValue(*iter_curentry, fmt);
				}
				sink("\n");
			}
			sink("}\n");
...
/* A lot of list_* functions following in common/simclist.h are exported (together with initialize, get_apdus etc.
 *(struct sm_module_operations)) from libsmm-local.so, which can't be used ad-hoc, if another SM module is present,
 * having to export sm_module_operations's functions as well (symbol name clash).
 * They are omitted therefore by using version(PATCH_EXPORTS) and not neccessarily required, otherwise an option might be to
 * patch libsmm-local.so related files to export the clashing names renamed
 */

version(PATCH_EXPORTS) {
extern(C) :
 ...
 */
void list_destroy(scope list_t* l);
 ...
 */
int list_attributes_comparator(scope list_t* l, element_comparator comparator_fun);
 ...
 * @see element_seeker()
 */
int list_attributes_seeker(scope list_t* l, element_seeker seeker_fun);
 ...
 */
int list_attributes_copy(scope list_t* l, element_meter metric_fun, int copy_data);
 ...
 */
int list_attributes_hash_computer(scope list_t* l, element_hash_computer hash_computer_fun);
 ...
 */
int list_attributes_serializer(scope list_t* l, element_serializer serializer_fun);
 ...
 * @see     list_restore_filedescriptor()
 */
int list_attributes_unserializer(scope list_t* l, element_unserializer unserializer_fun);
 ...
 */
int list_append(scope list_t* l, const(void)* data);
 ...
 * @return      1 for success. < 0 for failure
 */
int list_prepend(scope list_t* l, scope const(void)* data);
 ...
 */
void* list_fetch(scope list_t* l);
 ...
 * @return      reference to user datum, or NULL on errors
 */
void* list_get_at(scope const(list_t)* l, uint pos);
 ...
 */
void* list_get_max(scope const(list_t)* l);
 ...
 * @return      the reference to the element, or NULL
 */
void* list_get_min(scope const(list_t)* l);
 ...
 * @return      reference to user datum, or NULL on errors
 */
void* list_extract_at(scope list_t* l, uint pos);
 ...
 */
int list_insert_at(scope list_t* l, const(void)* data, uint pos);
 ...
 * @see list_delete_at()
 */
int list_delete(scope list_t* l, const(void)* data);
 ...
 * @return      0 on success. Negative value on failure
 */
int list_delete_at(scope list_t* l, uint pos);
 ...
 */
int list_delete_range(scope list_t* l, uint posstart, uint posend);
 ...
 */
uint list_size(scope const(list_t)* l);
 ...
 * @see list_size()
 */
int list_empty(scope const(list_t)* l);
 ...
 * @see list_get_at()
 */
int list_locate(scope const(list_t)* l, const(void)* data);
 ...
 */
void* list_seek(scope list_t* l, const(void)* indicator);
 ...
 * @see list_attributes_comparator()
 */
int list_contains(scope const(list_t)* l, const(void)* data);
 ...
 */
int list_concat(const list_t *l1, const list_t *l2, scope list_t* dest);
 ...
 * @see list_attributes_comparator()
 */
int list_sort(scope list_t* l, int versus);
 ...
 */
int list_iterator_start(scope list_t* l);
 ...
 */
void* list_iterator_next(scope list_t* l);
 ...
 */
int list_iterator_hasnext(scope const(list_t)* l);
 ...
 */
int list_iterator_stop(scope list_t* l);
 ...
 */
int list_hash(scope const(list_t)* l, scope list_hash_t* hash);
 ...
 * @see list_dump_filedescriptor()
 */
int list_dump_getinfo_filedescriptor(int fd, scope list_dump_info_t* info);
 ...
 * @see list_dump_filedescriptor()
 */
int list_dump_getinfo_file(scope const(char)* filename, scope list_dump_info_t* info);
 ...
 */
int list_dump_filedescriptor(scope const(list_t)* l, int fd, scope size_t* len);
 ...
 */
int list_dump_file(scope const(list_t)* l, scope const(char)* filename, scope size_t* len);
 ...
 * @return      0 if successful; -1 otherwise
 */
int list_restore_filedescriptor(scope list_t* l, int fd, scope size_t* len);
 ...
 * @return          0 if successful; -1 otherwise
 */
int list_restore_file(scope list_t* l, scope const(char)* filename, size_t* len);
} // version(SIMCLIST_DUMPRESTORE)
